...
const TestimonialSlider = ({ testimonials }) => {
  ...
  return (
    <div className="relative w-full max-w-screen-lg mx-auto py-2 flex flex-col items-center">
      <h2 className="text-3xl md:text-4xl font-bold text-center text-[#97144d] mb-12">
        Our Prime Corporate Customers
      </h2>
      
      <div className="relative w-full max-w-3xl bg-gradient-to-r from-pink-100 to-white shadow-2xl rounded-xl p-8 flex flex-col items-center text-center transform transition-all duration-500 hover:scale-105 border border-[#97144d]">
        <FaCapsules className="text-6xl text-[#97144d] absolute top-4 left-4 opacity-50" />
        <img
          src={testimonials[currentIndex].image}
          alt={testimonials[currentIndex].name}
          className="w-32 h-32 object-cover rounded-full border-4 border-[#97144d] shadow-lg mb-4"
        />
      ...
      <div className="flex justify-between mt-8 w-full max-w-xs">
        <button
          onClick={goToPrev}
          className="bg-[#97144d] text-white p-3 rounded-full shadow-lg hover:bg-gray-900 transition"
        >
          <FaArrowLeft size={20} />
        </button>
        <button
